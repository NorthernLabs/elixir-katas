defmodule ElixirKatas.TestFormatter do
  @moduledoc "Clone of `ExUnit.CLIFormatter` but with special handling for test failures."
  ...
  def handle_cast(
        {:test_finished, %ExUnit.Test{state: {:failed, failures}, tags: %{mfa: mfa}} = test},
        config
      ) do
    if config.trace do
